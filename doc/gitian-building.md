...
*Setup instructions for a Gitian build of SkullCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SkullCoin
...
- [Building SkullCoin Core](#building-skullcoin-core)
...
Clone the git repositories for skullcoin and Gitian.
...
in the skullcoin repository under 'Fetch and create inputs' to install sources which require
...
To build SkullCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the skullcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/skullcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/skullcoin-project/skullcoin
...
For example:
```bash
URL=https://github.com/thrasher-/skullcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit skullcoin=${COMMIT} --url skullcoin=${URL} ../skullcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit skullcoin=${COMMIT} --url skullcoin=${URL} ../skullcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit skullcoin=${COMMIT} --url skullcoin=${URL} ../skullcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the skullcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../skullcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/skullcoin
SIGPATH=/some/root/path/skullcoin-detached-sigs

./bin/gbuild --url skullcoin=${BTCPATH},signature=${SIGPATH} ../skullcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/skullcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/skullcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/skullcoin-osx-build.assert
```
...
[skullcoin-project/gitian.sigs.skc](https://github.com/skullcoin-project/gitian.sigs.skc/) repository, or if that's not possible create a pull
