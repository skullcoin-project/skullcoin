...
*Setup instructions for a Gitian build of Zeuscoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Zeuscoin
...
- [Building Zeuscoin Core](#building-zeuscoin-core)
...
Clone the git repositories for zeuscoin and Gitian.
...
in the zeuscoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Zeuscoin Core
----------------

To build Zeuscoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the zeuscoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zeuscoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/zeuscoin-project/zeuscoin
...
For example:
```bash
URL=https://github.com/thrasher-/zeuscoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit zeuscoin=${COMMIT} --url zeuscoin=${URL} ../zeuscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zeuscoin=${COMMIT} --url zeuscoin=${URL} ../zeuscoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zeuscoin=${COMMIT} --url zeuscoin=${URL} ../zeuscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the zeuscoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../zeuscoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/zeuscoin
SIGPATH=/some/root/path/zeuscoin-detached-sigs

./bin/gbuild --url zeuscoin=${BTCPATH},signature=${SIGPATH} ../zeuscoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zeuscoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zeuscoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/zeuscoin-osx-build.assert
```
...
[zeuscoin-project/gitian.sigs.zsc](https://github.com/zeuscoin-project/gitian.sigs.zsc/) repository, or if that's not possible create a pull
